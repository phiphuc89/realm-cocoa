...
/**
 ...
 connect to the Realm is automatically added to it. Any other roles you wish to use are
...
struct RLMRealmPrivileges {
    ...
    /// If `false`, no modifications to the Realm-level permissions are permitted. Write
    /// transactions can be performed locally, but any changes made will be reverted by
    /// the server.
    ///
...
struct RLMClassPrivileges {
    ...
    /// If `false`, no modifications to the Class-level permissions for this type are
    /// permitted. Write transactions can be performed locally, but any changes made will
    /// be reverted by the server.
    ///
    /// Note that if invalide privilege changes are made, `-[RLMRealm privilegesFor*:]`
    /// will return results reflecting those invalid changes until synchronization occurs.
    ///
